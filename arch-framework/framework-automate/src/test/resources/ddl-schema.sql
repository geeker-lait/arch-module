...
CREATE TABLE IF NOT EXISTS `user_relatives`(
    ...
    `sorted` int(2) COMMENT '顺序',
    `awardtime` date(255) COMMENT '颁发时间',
    `st` timestamp(255) COMMENT '时间戳',
    PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `user_job`(
    ...
    `sorted` int(2) COMMENT '顺序',
    `hiredate_time` date(255) COMMENT '入职时间',
    `dimission_time` date(255) COMMENT '离职时间',
    `st` timestamp(255) COMMENT '时间戳',
    PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `account_identifier`(
    ...
    `locked` boolean(1) COMMENT '是否被锁定，否认false',
...
CREATE TABLE IF NOT EXISTS `account_member`(
    ...
    `start_time` datetime(255) COMMENT '开始时间',
    `end_time` datetime(255) COMMENT '结束时间',
    PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `account_operate_log`(
    ...
    `operate_time` datetime(255) COMMENT '操作时间',
...
CREATE TABLE IF NOT EXISTS `market_name`(
    ...
    `pid` bigint(255) COMMENT '父ID',
    `market_typ` int(255) COMMENT '分类如：1:获客拉新、2:下单转化、3:提高客单、4:复购留存、5:....(数据字典获取)',
    `market_name` varchar(255) COMMENT '名称',
    `market_code` varchar(255) COMMENT '营销英文名',
    `icon` varchar(255) COMMENT '图标',
    `tier` int(255) COMMENT '层级',
    `sorted` int(255) COMMENT '排序',
    `descr` varchar(255) COMMENT '描述',
    `st` datime(255) COMMENT '时间戳',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='营销-名称';

CREATE TABLE IF NOT EXISTS `market_attrname`(
    `id` BIGINT(19) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `name_id` bigint(255) COMMENT '营销ID',
    `attr_name` varchar(255) COMMENT '属性名',
    `filed_name` varchar(255) COMMENT '字段名',
    `filed_typ` varchar(255) COMMENT '字段类型：string，int，long，decimal，double',
    `select_typ` int(255) COMMENT '属性选择的类型:0->唯一，1->单选，2->多选',
    `input_typ` int(255) COMMENT '属性录入方式:0->手工录入，1->从列表中选取',
    `unit` varchar(255) COMMENT '数字类型参数的单位，非数字类型可以为空',
    `st` datime(255) COMMENT '时间戳',
    PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `market_attrval`(
    `id` BIGINT(19) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `attr_name_id` bigint(255) COMMENT '属性ID',
    `attr_val` varchar(255) COMMENT '属性值',
    `defined_val` varchar(255) COMMENT '手动输入属性值（attrVal）的自定义值',
    `st` datime(255) COMMENT '时间戳',
    PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `market_master`(
    `id` BIGINT(19) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `id` bigint(255) COMMENT 'ID',
    `market_name_id` bigint(255) COMMENT '营销名称ID',
    `market_name` varchar(255) COMMENT '营销名称',
    `market_table` varchar(255) COMMENT '动态创建营销对应的表',
    ...
    `descr` varchar(255) COMMENT '使用说明',
    `h5` varchar(255) COMMENT 'h5链接',
    `start_time` (255) COMMENT '有效期/开始时间',
    `end_time` (255) COMMENT '失效期/结束时间',
...
CREATE TABLE IF NOT EXISTS `market_actor`(
    `id` BIGINT(19) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `market_id` bigint(255) COMMENT '营销ID',
    `market_typ` int(255) COMMENT '营销类型',
    `account_id` bigint(255) COMMENT '账号id',
    `rule_json` varchar(255) COMMENT '营销规则',
    `st` datatime(255) COMMENT '时间戳',
    PRIMARY KEY (`id`)
