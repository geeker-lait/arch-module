...
CREATE TABLE IF NOT EXISTS `user_master`(
    ...
    `uid` bigint(12) COMMENT '账号ID/用户ID/会员ID/商户ID',
    `nick` varchar(64) COMMENT '用户昵称可随机生成',
...
CREATE TABLE IF NOT EXISTS `rbac_client`(
    `id` BIGINT(19) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `role_name` varchar(32) COMMENT '角色名',
    `role_code` varchar(32) COMMENT '角色码',
    `icon` varchar(32) COMMENT '角色图标',
    `descr` varchar(32) COMMENT '角色描述',
    PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `rbac_verification`(
    `id` BIGINT(19) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `` (255) COMMENT '',
    `` (255) COMMENT '',
    `` (255) COMMENT '',
    `` (255) COMMENT '',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='用户验证';
