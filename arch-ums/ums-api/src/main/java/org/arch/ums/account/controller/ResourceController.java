...
public class ResourceController implements CrudController<Resource, java.lang.Long, ResourceSearchDto, ResourceService> {
    ...
    @NonNull
    public Response<List<Resource>> findByResourceIds(@PathVariable(value = "tenantId") Integer tenantId,
        ...
        try {
            return Response.success(resourceService.findAllBySpec(resourceWrapper));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
