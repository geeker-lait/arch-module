...
import org.arch.ums.biz.account.service.BizOauthTokenService;
...
public class OauthTokenBiz implements CrudBiz<OauthTokenRequest, OauthToken, Long, OauthTokenSearchDto, OauthTokenService> {

    private final TenantContextHolder tenantContextHolder;
    private final BizOauthTokenService bizOauthTokenService;
    ...
    public Response<Boolean> updateByIdentifierId(@RequestBody @Valid OauthToken oauthToken) {
        try {
            return Response.success(bizOauthTokenService.updateByIdentifierId(oauthToken));
        }
        catch (Exception e) {
    ...
    public Response<List<OauthTokenSearchDto>> find(@RequestBody @Valid OauthTokenRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(oauthTokenList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
