...
public class OperateLogBiz implements CrudBiz<OperateLogRequest, OperateLog, Long, OperateLogSearchDto, OperateLogService> {
    ...
    public Response<List<OperateLogSearchDto>> find(@RequestBody @Valid OperateLogRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(operateLogList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
