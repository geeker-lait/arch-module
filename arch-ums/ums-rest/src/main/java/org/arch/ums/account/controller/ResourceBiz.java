...
public class ResourceBiz implements CrudBiz<ResourceRequest, Resource, Long, ResourceSearchDto, ResourceService> {
    ...
    public Response<List<ResourceSearchDto>> find(@RequestBody @Valid ResourceRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(resourceList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
