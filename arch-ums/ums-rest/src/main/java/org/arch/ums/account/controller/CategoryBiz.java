...
public class CategoryBiz implements CrudBiz<CategoryRequest, Category, Long, CategorySearchDto, CategoryService> {
    ...
    public Response<List<CategorySearchDto>> find(@RequestBody @Valid CategoryRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(categoryList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
