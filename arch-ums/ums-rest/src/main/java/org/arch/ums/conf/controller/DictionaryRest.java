...
public class DictionaryRest implements CrudBiz<DictionaryRequest, Dictionary, Long, DictionarySearchDto,
        DictionaryService> {
    ...
    public Response<List<DictionarySearchDto>> find(@RequestBody @Valid DictionaryRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(dictionaryList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
