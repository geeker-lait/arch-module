...
public class PermissionBiz implements CrudBiz<PermissionRequest, Permission, Long, PermissionSearchDto, PermissionService> {
    ...
    public Response<List<PermissionSearchDto>> find(@RequestBody @Valid PermissionRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(permissionList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
