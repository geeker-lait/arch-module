...
public class JobBiz implements CrudBiz<JobRequest, Job, Long, JobSearchDto, JobService> {
    ...
    public Response<List<JobSearchDto>> find(@RequestBody @Valid JobRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(jobList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
