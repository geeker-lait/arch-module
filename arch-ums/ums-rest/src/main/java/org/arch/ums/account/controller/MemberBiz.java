...
public class MemberBiz implements CrudBiz<MemberRequest, Member, Long, MemberSearchDto, MemberService> {
    ...
    public Response<List<MemberSearchDto>> find(@RequestBody @Valid MemberRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(memberList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
