...
public class TicketBiz implements CrudBiz<TicketRequest, Ticket, Long, TicketSearchDto, TicketService> {
    ...
    public Response<List<TicketSearchDto>> find(@RequestBody @Valid TicketRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(ticketList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
