...
public class BankCardBiz implements CrudBiz<BankCardRequest, BankCard, Long, BankCardSearchDto, BankCardService> {
    ...
    public Response<List<BankCardSearchDto>> find(@RequestBody @Valid BankCardRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(bankCardList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
