...
public class IdCardBiz implements CrudBiz<IdCardRequest, IdCard, Long, IdCardSearchDto, IdCardService> {
    ...
    public Response<List<IdCardSearchDto>> find(@RequestBody @Valid IdCardRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(idCardList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
