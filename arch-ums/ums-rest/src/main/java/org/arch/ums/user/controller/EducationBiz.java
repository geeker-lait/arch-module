...
public class EducationBiz implements CrudBiz<EducationRequest, Education, Long,
    ...
    public Response<List<EducationSearchDto>> find(@RequestBody EducationRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(educationList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
