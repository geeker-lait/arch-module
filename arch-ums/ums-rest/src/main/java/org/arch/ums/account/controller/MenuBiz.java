...
public class MenuBiz implements CrudBiz<MenuRequest, Menu, Long, MenuSearchDto, MenuService> {
    ...
    public Response<List<MenuSearchDto>> find(@RequestBody @Valid MenuRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(menuList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
