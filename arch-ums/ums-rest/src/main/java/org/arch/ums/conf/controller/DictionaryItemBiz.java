...
public class DictionaryItemBiz implements CrudBiz<DictionaryItemRequest, DictionaryItem, Long, DictionaryItemSearchDto, DictionaryItemService> {
    ...
    public Response<List<DictionaryItemSearchDto>> find(@RequestBody @Valid DictionaryItemRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(dictionaryItemList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
