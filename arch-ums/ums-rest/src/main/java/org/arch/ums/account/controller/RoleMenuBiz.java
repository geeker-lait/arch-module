...
import org.arch.ums.biz.account.service.BizRoleMenuService;
...
public class RoleMenuBiz implements CrudBiz<RoleMenuRequest, RoleMenu, Long, RoleMenuSearchDto, RoleMenuService> {
    ...
    public Response<List<RoleMenuSearchDto>> find(@RequestBody @Valid RoleMenuRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(roleMenuList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
    ...
    public Response<Boolean> updateResourcesByRoleIdOfTenant(@PathVariable(value = "tenantId") Long tenantId,
        ...
        try {
            return Response.success(this.bizRoleMenuService.updateResourcesByRoleIdOfTenant(tenantId, roleId, menuIds));
        }
        catch (Exception e) {
            log.error(e.getMessage(),e);
            return Response.error(FAILED.getCode(), e.getMessage());
    ...
    public Response<Boolean> updateResourcesByRoleIdOfScopeId(@PathVariable(value = "scopeId") Long scopeId,
        ...
        try {
            return Response.success(this.bizRoleMenuService.updateResourcesByRoleIdOfScopeId(token.getTenantId()
                                                                                                  .longValue(),
                                                                                             scopeId,
                                                                                             roleId,
                                                                                             menuIds));
    ...
    public Response<List<MenuSearchDto>> findAllResourcesByRoleIdOfTenant(@PathVariable(value = "tenantId") Long tenantId,
                                                                          @PathVariable(value = "roleId") Long roleId) {
        try {
            List<Menu> menuList = this.bizRoleMenuService.findAllResourcesByRoleIdOfTenant(tenantId, roleId);
    ...
    public Response<List<MenuSearchDto>> findAllResourcesByRoleIdOfScopeId(@PathVariable(value = "scopeId") Long scopeId,
        ...
        try {
            ...
            List<Menu> menuList = this.bizRoleMenuService.findAllResourcesByRoleIdOfScopeId(tenantId, scopeId, roleId);
            List<MenuSearchDto> menuSearchDtoList = menuList.stream()
                                                            .map(menu -> {
                                                                MenuSearchDto menuSearchDto = new MenuSearchDto();
                                                                BeanUtils.copyProperties(menu, menuSearchDto);
