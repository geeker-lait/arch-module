...
public class TagBiz implements CrudBiz<TagRequest, Tag, Long, TagSearchDto, TagService> {
    ...
    public Response<List<TagSearchDto>> find(@RequestBody @Valid TagRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(tagList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
