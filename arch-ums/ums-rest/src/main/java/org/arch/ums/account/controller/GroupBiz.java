...
public class GroupBiz implements CrudBiz<GroupRequest, Group, Long, GroupSearchDto, GroupService> {
    ...
    public Response<List<GroupSearchDto>> find(@RequestBody @Valid GroupRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(groupList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
