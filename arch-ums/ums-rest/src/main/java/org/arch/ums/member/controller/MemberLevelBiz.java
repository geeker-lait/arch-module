...
public class MemberLevelBiz implements CrudBiz<MemberLevelRequest, MemberLevel, Long, MemberLevelSearchDto, MemberLevelService> {
    ...
    public Response<List<MemberLevelSearchDto>> find(@RequestBody @Valid MemberLevelRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(memberLevelList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
