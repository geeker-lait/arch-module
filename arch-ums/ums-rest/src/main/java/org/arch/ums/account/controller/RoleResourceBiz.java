...
import org.arch.ums.biz.account.service.BizRoleResourceService;
...
public class RoleResourceBiz implements CrudBiz<RoleResourceRequest, RoleResource, Long, RoleResourceSearchDto, RoleResourceService> {

    private final TenantContextHolder tenantContextHolder;
    private final BizRoleResourceService bizRoleResourceService;
    ...
    public Response<List<RoleResourceSearchDto>> find(@RequestBody @Valid RoleResourceRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(roleResourceList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
    ...
    public Response<Boolean> updateResourcesByRoleIdOfTenant(@PathVariable(value = "tenantId") Long tenantId,
        ...
        try {
            return Response.success(this.bizRoleResourceService.updateResourcesByRoleIdOfTenant(tenantId, roleId,
                                                                                                resourceIds));
        }
        catch (Exception e) {
            log.error(e.getMessage(),e);
    ...
    public Response<Boolean> updateResourcesByRoleIdOfScopeId(@PathVariable(value = "scopeId") Long scopeId,
        ...
        try {
            return Response.success(this.bizRoleResourceService.updateResourcesByRoleIdOfScopeId(token.getTenantId()
                                                                                                      .longValue(),
                                                                                                 scopeId,
                                                                                                 roleId,
                                                                                                 resourceIds));
    ...
    public Response<List<ResourceSearchDto>> findAllResourcesByRoleIdOfTenant(@PathVariable(value = "tenantId") Long tenantId,
                                                                              @PathVariable(value = "roleId") Long roleId) {
        try {
            List<Resource> resourceList = this.bizRoleResourceService.findAllResourcesByRoleIdOfTenant(tenantId, roleId);
    ...
    public Response<List<ResourceSearchDto>> findAllResourcesByRoleIdOfScopeId(@PathVariable(value = "scopeId") Long scopeId,
        ...
        try {
            ...
            List<Resource> resourceList = this.bizRoleResourceService.findAllResourcesByRoleIdOfScopeId(tenantId,
                                                                                                        scopeId,
                                                                                                        roleId);
            List<ResourceSearchDto> resourceSearchDtoList =
                    resourceList.stream()
