...
public class PostBiz implements CrudBiz<PostRequest, Post, Long, PostSearchDto, PostService> {
    ...
    public Response<List<PostSearchDto>> find(@RequestBody @Valid PostRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(postList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
