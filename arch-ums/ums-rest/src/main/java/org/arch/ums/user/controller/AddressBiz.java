...
public class AddressBiz implements CrudBiz<AddressRequest, Address, Long, AddressSearchDto, AddressService> {
    ...
    public Response<List<AddressSearchDto>> find(@RequestBody @Valid AddressRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(addressList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
