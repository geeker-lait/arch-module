...
public class PhoneBiz implements CrudBiz<PhoneRequest, Phone, Long, PhoneSearchDto, PhoneService> {
    ...
    public Response<List<PhoneSearchDto>> find(@RequestBody @Valid PhoneRequest request, TokenInfo token) {
        ...
        try {
            ...
            return Response.success(phoneList.stream().map(this::convertReturnDto).collect(Collectors.toList()));
        }
        catch (Exception e) {
            log.error(e.getMessage(), e);
            return Response.error(FAILED.getCode(), e.getMessage());
