...
public interface PayService<PC extends PayConfigStorage> {
     ...
     * @return http请求工具
     */
    RestTemplate getHttpRequestTemplate();
     ...
     *                           类型为{@link String }或者 {@link Date }，类型须强制限制，类型不对应则抛出异常{@link org.arch.payment.sdk.ex.PayException}
     ...
     * 接下来移除此方法
     *
     * @param tradeNoOrBillDate  支付平台订单号或者账单日期， 具体请 类型为{@link String }或者 {@link Date }，类型须强制限制，类型不对应则抛出异常{@link PayException}
     * @param outTradeNoBillType 商户单号或者 账单类型
     * @param transactionType    交易类型
     * @param callback           处理器
     * @param <T>                返回类型
