package org.arch.payment.core.http;
...
import org.arch.payment.core.bean.MethodType;
...
/*            String charset = "UTF-8";
            ...
            if (isJson(contentType[0], first)) {
                try {
                    ...
                    throw new PayException(new PayException("failure", String.format("类型转化异常,contentType: %s\n%s", entity.getContentType().getValue(), e.getMessage()), result));
                }
            }
            //xml类型
            ...
            throw new PayException(new PayException("failure", "类型转化异常,contentType:" + entity.getContentType().getValue(), result));
        }
        ...
        if (OutputStream.class.isAssignableFrom(responseType)) {
            try {
                ...
                throw new PayException(new PayException("InstantiationException", e.getMessage()));
            } catch (IllegalAccessException e) {
                throw new PayException(new PayException("IllegalAccessException", e.getMessage()));
            }
        }
        throw new PayException(new PayException("failure", "类型转化异常,contentType:" + entity.getContentType().getValue()));
    }
